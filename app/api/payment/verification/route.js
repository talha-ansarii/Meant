...
export async function POST(request) {
  ...
  if (webhookSignature === generatedSignature) {
    ...
    switch (event) {
     ...
     try {   const data = await Orders.create({
    ...
    console.log(data)
     ...
     try {
      const user = await User.findOne({ userId: body.payload.payment.entity.notes.userId });
      ...
      case 'payment.failed':
        ...
        console.log("Payment failed for order: ", body.payload.payment.entity.error_description);
